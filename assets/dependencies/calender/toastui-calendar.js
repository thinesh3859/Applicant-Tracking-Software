...
function isUsingDST(targetDate, timezoneName) {
  ...
  return Math.max(jan.getTimezoneOffset(), jul.getTimezoneOffset()) !== targetDate.getTimezoneOffset();
}
const dtfCache = {};
const timezoneNameValidationCache = {};

function isIntlDateTimeFormatSupported() {
  var _global, _global$Intl, _global$Intl$DateTime, _global$Intl$DateTime2;

  /**
   * Intl.DateTimeFormat & IANA Timezone Data should be supported.
   * also, hourCycle options should be supported.
   */
  return isFunction((_global = __webpack_require__.g) === null || _global === void 0 ? void 0 : (_global$Intl = _global.Intl) === null || _global$Intl === void 0 ? void 0 : (_global$Intl$DateTime = _global$Intl.DateTimeFormat) === null || _global$Intl$DateTime === void 0 ? void 0 : (_global$Intl$DateTime2 = _global$Intl$DateTime.prototype) === null || _global$Intl$DateTime2 === void 0 ? void 0 : _global$Intl$DateTime2.formatToParts);
}
...
const eventUIPropsKey = ['top', 'left', 'width', 'height', 'hasCollide', 'extraSpace', 'hidden', 'exceedLeft', 'exceedRight', 'croppedStart', 'croppedEnd', 'goingDurationHeight', 'modelDurationHeight', 'comingDurationHeight'];
/**
...
const datetime_MS_PER_DAY = 86400000;
...
function getRowStyleInfo(days, narrowWeekend, startDayOfWeek, workweek) {
  ...
  const dates = range_default()(startDayOfWeek, 7).concat(range_default()(days)).slice(0, 7);
...
function getTimezoneDifference(d) {
  let timezoneOffset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
...
const templates = {
  ...
  time(model) {
    ...
    if (start) {
      return v("span", null, v("strong", null, datetime_toFormat(start, 'HH:mm')), "\xA0", v("span", null, stripTags(title)));
    }
...
function createStoreContext() {
  ...
  function StoreProvider(_ref) {
    ...
    return v(StoreContext.Provider, {
      value: store,
      children
    });
  }
...
function Template(_ref) {
  ...
  return isString_default()(htmlOrVnode) ? v(tagName, {
...
function GridHeader(_ref) {
  ...
  }, dayNames.map((dayName, index) => v(DayName, {
    type: type,
...
function positionUIModels(start, end, matrices, iteratee) {
  const ymdListToRender = datetime_makeDateRange(start, end, datetime_MS_PER_DAY).map(date => datetime_toFormat(date, 'YYYYMMDD'));
  matrices.forEach(matrix => {
    matrix.forEach(column => {
      column.forEach((uiModel, index) => {
        ...
        const dateLength = datetime_makeDateRange(toStartOfDay(uiModel.getStarts()), toEndOfDay(uiModel.getEnds()), datetime_MS_PER_DAY).length;
        uiModel.top = index;
    ...
    matrix.forEach(row => {
      row.forEach((uiModel, col) => {
        ...
        for (let i = col + 1; i < maxRowLength; i += 1) {
          const collided = week_hasCollision(binaryMap[i - 1], startTime, endTime);
...
function getYMD(date) {
  let format = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'YYYYMMDD';
...
function getExceedDate(maxCount, eventsInDateRange, range) {
  const exceedDate = {};
  range.forEach(date => {
...
function excludeExceedEvents(matrices, visibleEventCount) {
...
Array.from(new Set(Object.values(eventMatrix).reduce((result, matrix3d) => result.concat(...flattenMatrix3d(matrix3d)), []))); // TODO: rename it. it is only used in the week & day view.


const getDayGridEvents = (row, calendarData, _ref3) => {
  let {
    narrowWeekend,
    hourStart,
    hourEnd
...
function getWeekDates(renderDate, _ref5) {
  ...
  return range_default()(startDayOfWeek, WEEK_DAYS + startDayOfWeek).reduce((acc, day) => {
...
function ExceedCount(_ref) {
  ...
  return exceedCount && !isClicked ? v("span", {
...
function CollapseButton(_ref2) {
  ...
  return isClicked && isClickedIndex ? v("span", {
    className: cls('weekday-exceed-in-week'),
...
const GridCells = compat_module_g(function GridCells(_ref) {
  ...
  return v(d, null, weekDates.map((cell, index) => {
...
const LayoutContainerContext = D(null);
...
function HorizontalEvent(_ref4) {
  ...
  return v("div", {
    ...
    param: uiModel.model
  })), !shouldHideResizeHandler ? v(HorizontalEventResizeIcon, {
...
function AlldayGridRow(_ref) {
  ...
  const horizontalEvents = F(() => events.filter(isWithinHeight(height, EVENT_HEIGHT + WEEK_EVENT_MARGIN_TOP)).map(uiModel => v(HorizontalEvent, {
  ...
  }, horizontalEvents), v(ResizingEventShadow, {
    weekDates: weekDates,
...
function EventDetailSectionDetail(_ref) {
  ...
  return v("div", {
    ...
    className: eventDetailSectionDetail_classNames.content
  }, (_calendar$name = calendar === null || calendar === void 0 ? void 0 : calendar.name) !== null && _calendar$name !== void 0 ? _calendar$name : '')), body && v("div", {
...
const FloatingLayerContext = D(null);
function FloatingLayerProvider(_ref) {
  ...
  return v(FloatingLayerContext.Provider, {
    value: floatingLayer
  }, children, v("div", {
...
function EventDetailPopup() {
  ...
  }, v(EventDetailSectionHeader, {
    event: event
  }), v(EventDetailSectionDetail, {
    event: event
  }), !isReadOnly && v("div", {
...
function CalendarSelector(_ref) {
  ...
  })), isOpened && v(CalendarDropdownMenu, {
    calendars: calendars,
    setOpened: setOpened,
...
const DateSelector = compat_module_x(function DateSelector(_ref, ref) {
  let {
    start,
    end,
    isAllday = false,
  ...
  return v(PopupSection, null, v("div", {
...
function EventStateSelector(_ref) {
  ...
  }, eventState === 'Busy' ? v(Template, {
    template: "popupStateBusy"
  }) : v(Template, {
    template: "popupStateFree"
  })), v("span", {
    className: eventStateSelector_classNames.arrowIcon
  })), isOpened && v(StateDropdownMenu, {
    setOpened: setOpened,
...
function EventFormPopup() {
  var _ref2, _formState$calendarId, _calendars$;
  ...
  const selectedCalendarId = (_ref2 = (_formState$calendarId = formState.calendarId) !== null && _formState$calendarId !== void 0 ? _formState$calendarId : event === null || event === void 0 ? void 0 : event.calendarId) !== null && _ref2 !== void 0 ? _ref2 : (_calendars$ = calendars[0]) === null || _calendars$ === void 0 ? void 0 : _calendars$.id;
  return compat_module_W(v("div", {
    role: "dialog",
  ...
  }, calendars !== null && calendars !== void 0 && calendars.length ? v(CalendarSelector, {
    selectedCalendarId: selectedCalendarId,
    calendars: calendars,
    formStateDispatch: formStateDispatch
  }) : v(PopupSection, null), v(TitleInputBox, {
    title: title,
    ...
    formStateDispatch: formStateDispatch,
    ref: datePickerRef
  }), v(EventStateSelector, {
    eventState: formState.state,
    formStateDispatch: formStateDispatch
  }), v(ClosePopupButton, {
    type: "form",
    close: close
  }), v(PopupSection, null, v(ConfirmPopupButton, null, isCreationPopup ? v(Template, {
    template: "popupSave"
  }) : v(Template, {
...
function SeeMoreEventsPopup() {
  ...
  }, events.map(uiModel => v(HorizontalEvent, {
...
function Layout(_ref) {
  ...
  return v(LayoutContainerProvider, {
    value: container
  }, v("div", {
    ref: containerRefCallback,
  ...
  }, container ? children : null), v(EventFormPopup, null), v(EventDetailPopup, null), v(SeeMoreEventsPopup, null), v(PopupOverlay, null));
}
...
const Panel = compat_module_x(function Panel(_ref2, ref) {
  let {
    name,
  ...
  return v(d, null, v("div", {
    ...
    style: styles,
    ref: ref
  }, children), isResizable ? v(PanelResizer, {
    name: name,
    width: resizerWidth,
...
function getStyles(_ref) {
  ...
  const containerStyle = {
    ...
    height: "calc(".concat(toPercent(height), " - ").concat(defaultMarginBottom, "px)"),
    top: toPercent(top),
    left: toPercent(left),
    borderRadius,
...
function TimeEvent(_ref3) {
  ...
  const onMoveStart = useDrag(DRAGGING_TYPE_CREATORS.moveEvent('timeGrid', "".concat(uiModel.cid())), {
    onDragStart: () => startDragEvent(timeEvent_classNames.moveEvent),
    onMouseUp: (e, _ref5) => {
      let {
        draggingState
      } = _ref5;
  ...
  return v("div", {
    "data-testid": "time-event-".concat(model.title, "-").concat(uiModel.cid()),
    "data-calendar-id": calendarId,
    "data-event-id": id,
  ...
  })) : null, comingDurationHeight ? v("div", {
    className: timeEvent_classNames.travelTime,
    style: comingDurationStyle
  }, v(Template, {
    template: "comingDuration",
    param: model
  })) : null, shouldShowResizeHandle ? v("div", {
...
function GridSelectionByColumn(_ref2) {
  ...
  const gridSelectionData = useStore(hooks_module_T(state => timeGridSelectionHelper.calculateSelection(state.gridSelection.timeGrid, columnIndex), [columnIndex]));
  ...
  return v(gridSelectionByColumn_GridSelection, gridSelectionProps);
}
...
function useTimeGridEventResize(_ref) {
  ...
  const baseResizingInfo = F(() => {
    ...
    let eventStartDateRowIndex = rows.findIndex(findRowIndexOf(startTZDate, eventStartDateColumnIndex)); // when it is -1, the event starts before the current view.
  ...
  const minimumHeight = F(() => baseResizingInfo ? timeGridData.rows[0].height : 0, [baseResizingInfo, timeGridData.rows]); // When drag an one-day event
  ...
  }, [baseResizingInfo, canCalculateGuideUIModel, columnIndex, currentGridPos, resizingStartUIModel, timeGridData.rows, minimumHeight]); // When drag a two-day event (but less than 24 hours)
  ...
  }, [baseResizingInfo, canCalculateGuideUIModel, columnIndex, currentGridPos, resizingStartUIModel, timeGridData.rows, minimumHeight]);
  useWhen(() => {
...
function VerticalEvents(_ref) {
  ...
  return v("div", {
    ...
    style: style
  }, eventUIModels.map(eventUIModel => v(TimeEvent, {
...
const Column = compat_module_g(function Column(_ref3) {
  ...
  }), v(ResizingGuideByColumn, {
    gridPositionFinder: gridPositionFinder,
    totalUIModels: totalUIModels,
    columnIndex: columnIndex,
    timeGridData: timeGridData
  }), v(GridSelectionByColumn, {
    columnIndex: columnIndex,
...
function getMovingEventPosition(_ref) {
  ...
  const millisecondsDiff = rowDiff * MS_PER_THIRTY_MINUTES + columnDiff * datetime_MS_PER_DAY;
...
function NowIndicatorLabel(_ref) {
  ...
  }, dateDifference !== 0 && v("span", {
    className: cls(nowIndicatorLabel_classNames.dayDifference)
  }, "[".concat(dateDifference > 0 ? '+' : '-').concat(Math.abs(dateDifference), "]")), v(Template, {
...
function HourRows(_ref) {
  ...
  }), showNowIndicator && isPresent(nowIndicatorState) && isPresent(zonedNow) && v(NowIndicatorLabel, {
    unit: "hour",
...
const TimeColumn = compat_module_g(function TimeColumn(_ref3) {
  ...
  }, !timezonesCollapsed && otherTimezoneHourRowsProps.map(rowsInfo => v(HourRows, {
...
function TimeGrid(_ref) {
  ...
  }).map((uiModelsByColumn, columnIndex) => setRenderInfoOfUIModels(uiModelsByColumn, setTimeStrToDate(columns[columnIndex].date, first(rows).startTime), setTimeStrToDate(columns[columnIndex].date, last(rows).endTime))), [columns, rows, events]);
  ...
  return v("div", {
    ...
    timeGridRows: rows
  }), v(movingEventShadow_MovingEventShadow, {
    gridPositionFinder: gridPositionFinder,
    timeGridData: timeGridData
  }), columns.map((column, index) => v(Column, {
  ...
  })), showNowIndicator && isPresent(currentDateData) && isPresent(nowIndicatorState) ? v(NowIndicator, {
    top: nowIndicatorState.top,
...
function TimezoneLabels(_ref2) {
  ...
  return v("div", {
    ...
    return v(TimezoneLabel, _extends({
  ...
  }), showTimezoneCollapseButton && v(TimezoneCollapseButton, {
    isCollapsed: timezonesCollapsed
  }), v(TimezoneLabel, _extends({
    width: timezoneLabelWidth,
...
function day_Day() {
  ...
  const gridRows = activePanels.map(key => {
    ...
    }, rowType === 'allday' ? v(AlldayGridRow, {
  ...
  return v(Layout, {
    ...
    rowStyleInfo: rowStyleInfo
  })), gridRows, activePanels.includes('time') ? v(Panel, {
    name: "time",
    autoSize: 1,
    ref: setTimePanelRef
  }, v(TimeGrid, {
...
function AccumulatedGridSelection(_ref) {
  ...
  }, gridSelectionDataByRow.map(gridSelectionData => gridSelectionData ? v(GridSelection, {
    type: "accumulated",
...
function CellHeader(_ref2) {
  ...
  return v("div", {
    ...
    param: templateParam
  })), exceedCount ? v(MoreEventsButton, {
    type: type,
    number: exceedCount,
...
const MonthEvents = compat_module_g(function MonthEvents(_ref) {
  ...
  const dayEvents = events.filter(isWithinHeight(contentAreaHeight, eventHeight + MONTH_EVENT_MARGIN_TOP)).map(uiModel => v(HorizontalEvent, {
...
function useDayGridMonthEventMove(_ref) {
  ...
  useWhen(() => {
    ...
    if (shouldUpdate) {
      ...
      const timeOffsetPerDay = getDateDifference(currentDate, preStartDate) * datetime_MS_PER_DAY;
...
function DayGridMonth(_ref) {
  ...
  return v("div", {
    ...
    return v("div", {
      ...
      eventHeight: MONTH_EVENT_HEIGHT,
      className: cls('weekday-events')
    }), v(GridSelectionByRow, {
      weekDates: week,
      narrowWeekend: narrowWeekend,
      rowIndex: rowIndex
    }), v(AccumulatedGridSelection, {
      rowIndex: rowIndex,
      weekDates: week,
      narrowWeekend: narrowWeekend
    })), v(ResizingGuideByRow, {
      dateMatrix: dateMatrix,
      gridPositionFinder: gridPositionFinder,
      rowIndex: rowIndex,
      cellWidthMap: cellWidthMap,
      renderedUIModels: renderedEventUIModels
    }), v(dayGridMonth_movingEventShadow_MovingEventShadow, {
      dateMatrix: dateMatrix,
      gridPositionFinder: gridPositionFinder,
      rowIndex: rowIndex,
      rowInfo: rowInfo
...
function Week() {
  ...
  return v(Layout, {
    ...
    options: weekOptions,
    rowStyleInfo: rowStyleInfo
  })), dayGridRows, hasTimePanel ? v(Panel, {
    name: "time",
    autoSize: 1,
    ref: setTimePanelRef
  }, v(TimeGrid, {
...
function CalendarContainer(_ref) {
  ...
  return v(EventBusProvider, {
    value: eventBus
  }, v(ThemeProvider, {
    store: theme
  }, v(StoreProvider, {
    store: store
  }, v(FloatingLayerProvider, null, children))));
}
...
class CalendarCore {
  ...
  renderToString() {
    return dist(v(CalendarContainer, {
      theme: this.theme,
      store: this.store,
      eventBus: this.eventBus
    }, this.getComponent()));
